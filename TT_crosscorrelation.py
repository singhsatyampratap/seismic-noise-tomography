...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 14:09:04 2020
...
class rotate_seed:
    ...
    def open_xml(self):
        ...
        for i in range(0,32):
            ...
            #stn1='CM{:02}'.format(i+1)
            #staname1='XB.{}N.xml'.format(stn1)
            
            #inv[0][0].code=u'CM{:02}N'.format(i+1)
            #inv.write(staname1, format="stationxml", validate=True)
            
            
            #stn2='CM{:02}'.format(i+1)
            #staname2='XB.{}E.xml'.format(stn2)
            
            #inv[0][0].code=u'CM{:02}E'.format(i+1)
            #inv.write(staname2, format="stationxml", validate=True)
        return (self.m)
...
rot_station_names=['CM{:02}'.format(i)  for i in range(1,33)]
rot_base_station='CM01' 
lat_lon=rotate_seed().open_xml()
dict_st_latlon={rot_station_names[i]:lat_lon[i,:] for i in range(32) }

lat1=dict_st_latlon[rot_base_station][0]
lon1=dict_st_latlon[rot_base_station][1]


ba={rot_station_names[j]:rotate_seed().back_azimuth(lat1, lon1, lat_lon[j,0], lat_lon[j,1]) for j in range(32)}


def rotate_it(traces,backazimuthdic,month_stations):
    rotated_traces=[]
    st=obspy.Stream([obspy.Trace()])
    for i in range(0,len(month_stations),2):
    #for month_station in month_stations:
        st=obspy.Stream([obspy.Trace()])
        #print month_stations[i].name
        for trace in traces:
            #print trace.stats.station
            if str(trace.stats.station)==month_stations[i].name:
                st+=obspy.Stream([trace])
                ba=backazimuthdic[str(trace.stats.station)]
    
        #if len(st)==3:
        try:
            st=st[1:]
            st.rotate(method="NE->RT", back_azimuth=ba)
            rotated_traces.append(st[1])
            rotated_traces.append(st[0])
        except:
            pass
    #rotated_traces_comp=rotated_traces_comp.append(rotated_traces)
...
for date in dates:
    ...
    traces=rotate_it(traces,ba,month_stations)
    
    
    #satyam=month_stations
    #month_stations2= change_month_station(month_stations)
...
/ PERIOD_RESAMPLE)
        ...
        pairs=[(k1,k2) for k2 in listtracedict]
